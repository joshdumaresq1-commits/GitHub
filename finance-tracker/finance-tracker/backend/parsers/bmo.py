...
class BMOParser(BaseParser):
    """Parser for BMO (Bank of Montreal) statements."""

    bank_name = "BMO"

    @classmethod
    def detect(cls, text: str) -> bool:
        return bool(re.search(r"Bank of Montreal|BMO(?!\s+Harris)|bmo\.com", text, re.IGNORECASE))

    def parse(self, pdf_path: Path) -> list[dict]:
        full_text = self.get_text(pdf_path)
        if re.search(r"mastercard|credit card|credit account|visa", full_text, re.IGNORECASE):
    ...
    def _parse_chequing(self, full_text: str) -> list[dict]:
        ...
        skip = re.compile(
            ...
            r"bmo|bank of montreal|account|statement|page|branch|transit|total)",
            re.IGNORECASE,
        )
        ...
        for line in lines:
            ...
            if m:
                ...
                val3 = self.clean_amount(col3) if col3 else 0.0
                val4 = self.clean_amount(col4) if col4 else 0.0
                ...
                if col5:
                    ...
                    if val3 > 0:
                        ...
                        amount_cents = self.to_cents(-val4)
                else:
                    amount_cents = self.to_cents(val3)
    ...
    def _parse_credit(self, full_text: str) -> list[dict]:
        ...
        skip = re.compile(
            ...
            r"bmo|bank of montreal|account|statement|page|opening|closing|total|"
